...
while (@i <= @n)
begin
	--1. lấy ra mã số nhân viên thứ @i
	set @maso =cast( (select manv from nhanvien where cast(manv as int) = @i)  as int)

	--2. nhân viên thứ i có manv là số chẵn thì xuất ra
	if( @maso % 2 = 0 )
	begin
...
begin
	...
	--nhân viên thứ i có manv là số chẵn và khác 4 thì xuất ra
...
---------------------------
/*
làm khối lệnh trong try......nếu ko có lỗi thì ko làm trong catch
NGƯỢC LẠI NẾU GẶP LỖI THÌ BAY THẰNG VÀO CATCH LÀM
*/
-- cách 2 ngắn gọn
BEGIN TRY
	INSERT PHONGBAN
	VALUES ('keToan',10,'003','2020-06-02')
	 -- Nếu lệnh chèn thực thi thành công in ra dòng bên dưới
	PRINT 'them thanh cong......SUCCESS: Record was inserted.'
END TRY
-- Nếu có lỗi xảy ra khi chèn dữ liệu in ra dòng thông báo lỗi cùng với thông tin mã lỗi và thông báo lỗi
BEGIN CATCH
	PRINT 'co loi xay ra'--'FAILURE: Record was not inserted.'
